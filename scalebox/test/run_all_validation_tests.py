...
class TestSuiteRunner:
    ...
    def run_all_tests(self, selected_tests: List[str] = None) -> Dict[str, Any]:
        ...
        # 过滤选定的测试
        if selected_tests:
            test_suites = [suite for suite in test_suites if suite['module'] in selected_tests]
    ...
    def generate_final_report(self, total_duration: float) -> Dict[str, Any]:
        ...
        total_tests = sum(r['details'].get('total_tests', 0) for r in self.results.values())
        ...
        logger.info(f"  测试成功率: {(total_passed/total_tests*100):.1f}%" if total_tests > 0 else "  测试成功率: N/A")
        ...
        for module, result in self.results.items():
            ...
            if 'total_tests' in result['details']:
                logger.info(f"    测试: {result['details']['passed']}/{result['details']['total_tests']}")
        ...
        # 性能对比（如果有同步和异步测试结果）
        if 'test_sandbox_async_comprehensive' in self.results and 'test_sandbox_sync_comprehensive' in self.results:
            async_duration = self.results['test_sandbox_async_comprehensive']['duration']
