...
class StressTestValidator:
    ...
    async def test_large_file_operations_async(self):
        ...
        try:
            ...
            # 写入大文件
            start_time = time.time()
            result = await sandbox.files.write("/tmp/large_file_async.txt", large_content)
            write_duration = time.time() - start_time
        ...
        try:
            def worker(worker_id: int) -> dict:
                ...
                for i in range(10):
                    ...
                    # 命令执行
                    result = sandbox.commands.run(f"echo 'Worker {worker_id}, Command {i}'")
        ...
        try:
            async def worker(worker_id: int) -> dict:
                ...
                for i in range(10):
                    ...
                    # 命令执行
                    result = await sandbox.commands.run(f"echo 'Async Worker {worker_id}, Command {i}'")
                    assert result.exit_code == 0
    ...
    def test_edge_cases_sync(self):
        ...
        try:
            ...
            # 深层目录测试
            deep_path = "/tmp/" + "/".join([f"dir{i}" for i in range(10)]) + "/deep_file.txt"
    ...
    async def test_edge_cases_async(self):
        ...
        try:
            ...
            read_binary = await sandbox.files.read("/tmp/binary_test.bin", format="bytes")
    ...
    async def test_error_recovery_async(self):
        ...
        try:
            ...
            for i in range(5):
                ...
                # 错误后继续正常操作
                await sandbox.files.write(f"/tmp/async_recovery_test_{i}.txt", f"Async recovery test {i}")
            ...
            async def error_and_recovery(index):
                ...
                # 正常操作
                await sandbox.files.write(f"/tmp/concurrent_recovery_{index}.txt", f"Concurrent {index}")
                content = await sandbox.files.read(f"/tmp/concurrent_recovery_{index}.txt")
    ...
    async def test_resource_management_async(self):
        ...
        try:
            ...
            async def setup_sandbox(sandbox, index):
                result = await sandbox.commands.run(f"echo 'Async Sandbox {index} test'")
                assert result.exit_code == 0
                
                await sandbox.files.write(f"/tmp/async_resource_test_{index}.txt", f"Async resource test {index}")
                return sandbox
            
            setup_tasks = [setup_sandbox(sandbox, i) for i, sandbox in enumerate(sandboxes)]
            ...
            async def verify_sandbox(sandbox, index):
                content = await sandbox.files.read(f"/tmp/async_resource_test_{index}.txt")
                assert content == f"Async resource test {index}"
                return True
            
            verify_tasks = [verify_sandbox(sandbox, i) for i, sandbox in enumerate(sandboxes)]
    ...
    def run_all_tests(self):
        ...
        # 大数据处理测试
        self.run_test(self.test_large_file_operations_sync, "Large File Operations Sync")
        self.run_test(self.test_large_file_operations_async, "Large File Operations Async", is_async=True)
        
        # 并发压力测试
        self.run_test(self.test_concurrent_sync_operations, "Concurrent Sync Operations")
        self.run_test(self.test_concurrent_async_operations, "Concurrent Async Operations", is_async=True)
        
        # 内存压力测试
        self.run_test(self.test_memory_stress_sync, "Memory Stress Sync")
        self.run_test(self.test_memory_stress_async, "Memory Stress Async", is_async=True)
        ...
        self.run_test(self.test_error_recovery_async, "Error Recovery Async", is_async=True)
        
        # 资源管理测试
        self.run_test(self.test_resource_management_sync, "Resource Management Sync")
        self.run_test(self.test_resource_management_async, "Resource Management Async", is_async=True)
    ...
    def print_summary(self):
        ...
        # 性能统计
        sync_tests = [r for r in self.test_results if 'Sync' in r['test'] and r['success']]
        async_tests = [r for r in self.test_results if 'Async' in r['test'] and r['success']]
