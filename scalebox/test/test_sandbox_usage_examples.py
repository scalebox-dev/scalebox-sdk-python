...
class SandboxManager:
    ...
    def __init__(self, template: str = "base", max_concurrent: int = 5):
        ...
        self._lock = asyncio.Lock() if asyncio.iscoroutinefunction(self.__class__) else None
        ...
        async def wrapper(*args, **kwargs):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    last_exception = e
                    if attempt < max_retries - 1:
                        logger.warning(f"异步尝试 {attempt + 1}/{max_retries} 失败: {e}, {delay}秒后重试")
...
class CodeExecutionService:
    ...
    async def async_execute_python_code(self, code: str, timeout: int = 30) -> Dict[str, Any]:
        """异步执行Python代码"""
...
class FileProcessingService:
    ...
    def process_files_batch(self, files_data: List[Dict[str, str]]) -> List[Dict[str, Any]]:
        """批量处理文件"""
    ...
    async def async_process_files_batch(self, files_data: List[Dict[str, str]]) -> List[Dict[str, Any]]:
        """异步批量处理文件"""
...
class InteractiveSession:
    ...
    def get_session_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'session_duration': self.session_log[-1][2] - self.session_log[0][2] if self.session_log else 0,
...
class OptimizedFileOperations:
    ...
    @staticmethod
    async def batch_file_operations_async(sandbox: AsyncSandbox, operations: List[Dict[str, Any]]):
        """异步批量文件操作优化"""
        # 按操作类型分组
        writes = [op for op in operations if op['type'] == 'write']
        reads = [op for op in operations if op['type'] == 'read']
...
class AsyncCommandExecutionPool:
    ...
    async def execute_commands_concurrent(self, commands: List[str]) -> List[Dict[str, Any]]:
        """异步并发执行命令"""
...
async def demo_async_code_execution_service():
    ...
    for i, result in enumerate(results):
        logger.info(f"任务 {i + 1} - 退出码: {result['exit_code']}, 输出: {result['stdout'].strip()}")
...
def demo_file_processing_service():
    ...
    test_files = [
        ...
        {
            'name': 'data.json',
            'content': json.dumps({'key': 'value', 'numbers': [1, 2, 3], 'nested': {'a': 1}})
        },
        {
            'name': 'invalid.json',
...
def demo_performance_optimization():
    ...
    with SandboxManager().get_sandbox() as sandbox:
        ...
        for i in range(10):
            ...
            operations.append({
                'type': 'read',
                'path': f'/tmp/read_test_{i}.txt'
            })
        
        # 测试优化的批量操作
        start_time = time.time()
        results = OptimizedFileOperations.batch_file_operations_sync(sandbox, operations)
        duration = time.time() - start_time
...
async def demo_async_performance_optimization():
    ...
    async with AsyncSandboxManager().get_sandbox() as sandbox:
        ...
        # 先写入一些文件用于读取测试
        write_ops = [{"path": f'/tmp/async_read_test_{i}.txt', "data": f'Async read test {i}'} for i in range(20)]
        ...
        for i in range(20):
            operations.append({
                'type': 'read',
                'path': f'/tmp/async_read_test_{i}.txt'
            })
        
        # 测试异步优化的批量操作
        start_time = time.time()
        results = await OptimizedFileOperations.batch_file_operations_async(sandbox, operations)
        duration = time.time() - start_time
